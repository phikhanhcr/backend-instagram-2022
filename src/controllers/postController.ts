import { EVENT_ACTION_ON_POST } from "./../event/PostEvent";
...
class Post {
  ...
  public getPostByUserId = async (req, res) => {
    // get post by userId
    const userId = req.user;
    const data = await PostRepository.getPostsFromUserId(
  ...
  public likePost = async (req, res) => {
    ...
    if (targetPost.length > 0) {
      ...
      const updatedPost = await PostRepository.update(post_id, {
        $push: { like_list: userId },
        $inc: { like_count: 1 },
      });

      console.log({ updatedPost });

      eventbus.emit(EVENT_ACTION_ON_POST, updatedPost, userId);
  ...
  public unlikePost = async (req, res) => {
    ...
    if (targetPost.length > 0) {
      ...
      targetPost[0].like_list = targetPost[0].like_list.filter(
      ...
      return res
        .status(400)
        .json({ msg: "You have not liked this user yet!!!" });
    }
  };
}
