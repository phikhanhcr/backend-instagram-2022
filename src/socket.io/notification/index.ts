...
let notificationSocket = (io) => {
  io.on("connection", (socket) => {
    ...
    socket.on("notify-comment_on_post", async (data) => {
      ...
      const specificNotify = await NotificationModel.findById(
        newNotification._id
      ).lean();

      if (
        clients[post.userId._id] &&
        data.user_commented_id != post.userId._id
      ) {
    ...
    socket.on("notify-comment_replied_on_post", async (data) => {
      ...
      if (post.userId._id.toString() === data.reply_to._id.toString()) {
        ...
        if (
          ...
          clients[post.userId._id].forEach((e) => {
            io.to(e).emit(
              "response-notify-comment_replied_on_post",
              specificNotify
            );
          });
        }
        ...
        const newNotificationSendRootComment =
        ...
        if (clients[data.reply_to._id]) {
          const specificNotifySendRootComment =
            await NotificationModel.findById(newNotificationSendRootComment._id)
              .populate("sender", "username avatar")
              .lean();
          clients[data.reply_to._id].forEach((e) => {
            io.to(e).emit(
              "response-notify-comment_replied_on_post",
              specificNotifySendRootComment
            );
          });
        }
      }
    ...
    socket.on("handle-post-unlike", async (data) => {
      const { receiver, rootContent, sender } = data;
      // remove notification
      const check = await NotificationRepository.removeNotify(
        rootContent._id,
        receiver,
        sender,
        "like"
